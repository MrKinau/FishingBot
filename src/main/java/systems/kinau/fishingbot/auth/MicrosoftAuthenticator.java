...
public class MicrosoftAuthenticator implements IAuthenticator {
    ...
    private final AuthDialogHelper authDialogHelper = new AuthDialogHelper();
        ...
        try {
            if (refreshToken == null) {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    authDialogHelper.closeDialog(authDialog);
                    if (!FishingBot.getInstance().getCurrentBot().isNoGui()) {
            ...
            authDialogHelper.setDialogProgress(authDialog, 0.1);
            ...
            authDialogHelper.setDialogProgress(authDialog, 0.2);
            ...
            authDialogHelper.setDialogProgress(authDialog, 0.5);
            ...
            authDialogHelper.setDialogProgress(authDialog, 0.9);
...
class AuthDialogHelper {
        public void setDialogProgress(CompletableFuture<Alert> authDialog, double progress) {
    ...
    public void closeDialog(CompletableFuture<Alert> authDialog) {
        modifyDialog(authDialog, alert -> {
    ...
    public void modifyDialog(CompletableFuture<Alert> authDialog, Consumer<Alert> consumer) {
        if (authDialog != null) {
